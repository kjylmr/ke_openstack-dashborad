...
#from a10_neutron_lbaas.a10_config import A10Config
...
class AddMemberAction(workflows.Action):
    ...
    def __init__(self, request, *args, **kwargs):
        ...
        try:
            ...
            LOG.info("355:------------------------------kejunyang------------------------------pools=%s" %(pools))
        except Exception:
            pools = []
            exceptions.handle(request,
...
class AddPMAssociationAction(workflows.Action):
    ...
    def populate_monitor_id_choices(self, request, context):
        ...
        try:
            ...
            elif pool['provider'] == 'a10networks':
                monitor_id_choices.append(('ping', 'PING  interval:5 retry:3 timeout:5'))
                #a10_monitors = api.lbaas.get_a10_healthmonitor()
                #monitor_id_choices.extend(a10_monitors)
        except Exception:
            exceptions.handle(request,
